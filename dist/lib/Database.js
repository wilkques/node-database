...
class Database {
     ...
     * @returns 查询构建器实例
     */
    static async connect(driver, host = "localhost", username = "", password = "", database = "", port, charset = "utf8mb4") {
        let config;
        ...
        if (!config.port) {
            switch (config.driver) {
                ...
                default: {
                    throw new Error(`Unsupported database driver: ${config.driver}. Supported drivers: mysql, postgres, sqlite`);
                }
            }
        }
        ...
        try {
            ...
            if (error.code === "MODULE_NOT_FOUND") {
                ...
                throw new Error(`Database driver '${config.driver}' not found. Please install the required package:\n` +
        ...
        try {
            ...
            throw new Error(`Failed to connect to ${config.driver} database: ${error.message}`);
        }
     ...
     * @returns 查询构建器实例
     */
    static createBuilder(connection, grammar) {
        const builder = new Builder(connection, grammar);
        builder.setProcessor(new Processor());
        return builder;
    }
     ...
     * @returns 事务查询构建器
     */
    static async transaction(config) {
     ...
     * @returns 查询结果
     */
    static async raw(config, sql, bindings = []) {
        const builder = await Database.connect(config);

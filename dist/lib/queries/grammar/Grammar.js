...
export default class Grammar {
    ...
    if (
      ...
      (!query.queries ||
        Object.values(query.queries).every(
          (q) => !q.queries || q.queries.length === 0,
        ))
    ) {
      actualQuery = {
        queries: {
          columns: {
            queries: (query.components.columns || []).map((col) =>
          ...
          groups: {
            queries: (query.components.groups || []).map((group) =>
  ...
  compileGroups(query) {
    ...
    const groups = query.queries.groups.queries.map((group) =>
      this.wrap(group),
    );
  ...
  compileComponents(builder) {
    ...
    const query = builder.components
      ? {
          queries: {
            columns: {
              queries: (builder.components.columns || []).map((col) =>
                typeof col === "string" ? col : col.column || col,
              ),
            },
            froms: {
            ...
            groups: {
              queries: (builder.components.groups || []).map((group) =>
                typeof group === "string" ? group : group.column || group,
              ),
            },

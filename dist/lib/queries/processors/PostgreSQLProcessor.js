...
export default class PostgreSQLProcessor extends Processor {
    ...
    processColumnListing(results) {
        if (!results || !Array.isArray(results))
            return [];
        return results.map((row) => row.column_name || row.columnname || row.attname).filter(Boolean);
    }
    /**
    ...
    processAggregate(results, column) {
        ...
        const value = row[column || 'aggregate'] || (column ? row[column.toLowerCase()] : null) ||
            row.count || row.sum || row.avg || row.max || row.min ||
            Object.values(row)[0];
...
//# sourceMappingURL=PostgreSQLProcessor.js.map
...
export class DatabaseErrorFactory {
   ...
   */
  static fromError(error: any, context: {
    driver?: string;
    sql?: string;
    bindings?: any[];
    operation?: string;
    ...
    if (error.code === 'MODULE_NOT_FOUND' || message.includes('driver') && message.includes('not found')) {
      return new DriverNotFoundError(context.driver || 'unknown');
    }

    if (error.code && error.code.startsWith('ECONNECT') || message.includes('connect')) {

...
export default class Processor implements ProcessorInterface {
     ...
     */
    async processInsertGetId(builder: any, data: any, sequence?: string): Promise<number> {
        if (!builder) {
    ...
    protected mapColumnType(type: string): string {
        ...
        if (lowerType.includes('float') || lowerType.includes('double') || lowerType.includes('decimal')) return 'float';
        if (lowerType.includes('bool')) return 'boolean';
        if (lowerType.includes('date') || lowerType.includes('time')) return 'datetime';
        if (lowerType.includes('text') || lowerType.includes('varchar') || lowerType.includes('char')) return 'string';

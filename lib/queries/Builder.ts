...
export class Builder implements QueryBuilder {
     ...
     * @returns This builder instance
     */
    whereSub(column: string, operator: string, callback: Function | Builder, andOr: string = 'and'): this {
     ...
     * @returns This builder instance
     */
    whereExists(callback: Function | Builder, andOr: string = 'and', not: boolean = false): this {
        const [sql, bindings] = this.createSub(callback);
    ...
    join(table: string, first: string | Function, operator: string = '=', second?: string): JoinBuilder {
        ...
        return joinBuilder.processJoin(table, first, operator, second || first, 'inner');
    }
    ...
    leftJoin(table: string, first: string | Function, operator: string = '=', second?: string): JoinBuilder {
        ...
        return joinBuilder.processJoin(table, first, operator, second || first, 'left');
    }
    ...
    rightJoin(table: string, first: string | Function, operator: string = '=', second?: string): JoinBuilder {
        ...
        return joinBuilder.processJoin(table, first, operator, second || first, 'right');
    }
    ...
    fullJoin(table: string, first: string, operator: string = '=', second?: string): JoinBuilder {
        ...
        return joinBuilder.processJoin(table, first, operator, second || first, 'full');
    }
     ...
     * @returns JoinBuilder for continued method chaining
     */
    joinWhere(table: string, first: string, operator: string = '=', second?: string, type: 'inner' | 'left' | 'right' | 'full' | 'cross' = 'inner'): JoinBuilder {
        const JoinBuilderClass = this._getJoinBuilderClass();
        ...
        return joinBuilder.processJoin(table, first, operator, second || first, type);
    }
     ...
     */
    orderBySub(query: Function | Builder, direction: 'asc' | 'desc' = 'asc'): this {
    ...
    async insertSub(columns: string[], query: Function | Builder): Promise<any> {
        ...
        const wrappedColumns = columns.map(col => this.contactBacktick(col)).join(', ');
     ...
     */
    prepareSubQueryArgs(operator: string, callback: Function | Builder, useDefault: boolean = false): [Function | Builder, string] {
        if (useDefault) {
            // For 2-parameter calls: column, callback -> callback, '='
            return [callback, '='];
        }
    ...
    private copyStateTo(target: Builder): void {
        ...
        // Copy queries
        target.queries = {
            columns: { queries: [...this.queries.columns.queries], bindings: [...this.queries.columns.bindings] },
            froms: { queries: [...this.queries.froms.queries], bindings: [...this.queries.froms.bindings] },
            joins: { queries: [...this.queries.joins.queries], bindings: [...this.queries.joins.bindings] },
            wheres: { queries: [...this.queries.wheres.queries], bindings: [...this.queries.wheres.bindings] },
            groups: { queries: [...this.queries.groups.queries], bindings: [...this.queries.groups.bindings] },
            havings: { queries: [...this.queries.havings.queries], bindings: [...this.queries.havings.bindings] },
            orders: { queries: [...this.queries.orders.queries], bindings: [...this.queries.orders.bindings] },
            limits: { queries: [...this.queries.limits.queries], bindings: [...this.queries.limits.bindings] },
            offset: { queries: [...this.queries.offset.queries], bindings: [...this.queries.offset.bindings] },
            insert: { queries: [...this.queries.insert.queries], bindings: [...this.queries.insert.bindings] },
            update: { queries: [...this.queries.update.queries], bindings: [...this.queries.update.bindings] },
            unions: { queries: [...this.queries.unions.queries], bindings: [...this.queries.unions.bindings] }
        };
    ...
    private _getJoinBuilderClass(): any {
        try {
            return typeof globalThis !== 'undefined' && (globalThis as any).JoinBuilderClass
                ? (globalThis as any).JoinBuilderClass : null;
        } catch {
            return null;
        }
    }
...
class CaseBuilderImpl implements CaseBuilder {
     ...
     */
    private extractConditionOrSubquery(builder: Builder): { condition: string; isSubquery: boolean } {
        try {
    ...
    when(condition: any, value: any): this {
        ...
        for (const condition of this.conditions) {
            if (typeof condition.when === 'function') {
                ...
                const { condition: conditions, isSubquery } = this.extractConditionOrSubquery(subBuilder);

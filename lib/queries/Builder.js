...
export default class Builder {
    constructor(connection, grammar = null, processor = null) {
        ...
        this.methods = {
            set: [
                'table', 'username', 'password', 'database',
                'host', 'raw', 'from'
            ],
            process: [
                'insertGetId'
        ...
        this.bindingComponents = [
            ...
            'wheres', 'groups', 'havings', 'orders', 'limits', 'unions'
        ];
    ...
    async _setDatabaseSpecificComponents(driver) {
        try {
            switch (driver) {
                ...
                case 'mysql2':
                    {
                        const { default: MySQLGrammar } = await import('./grammar/MySQL.js');
                        const { default: MySQLProcessor } = await import('./processors/MySQLProcessor.js');
                ...
                case 'pg':
                    {
                        const { default: PostgreSQLGrammar } = await import('./grammar/PostgreSQL.js');
                        const { default: PostgreSQLProcessor } = await import('./processors/PostgreSQLProcessor.js');
                ...
                case 'sqlite3':
                    {
                        const { default: SQLiteGrammar } = await import('./grammar/SQLite.js');
                        const { default: SQLiteProcessor } = await import('./processors/SQLiteProcessor.js');
                ...
                default:
                    // Use base classes as fallback
                    {
                        ...
                        const { default: Processor } = await import('./processors/Processor.js');
            ...
            console.warn(`Failed to load database-specific components for driver "${driver}":`, error.message);
            ...
            try {
                ...
                console.error('Failed to load fallback grammar/processor:', fallbackError.message);
    ...
    from(froms, as = null) {
        ...
        if (Array.isArray(froms)) {
            ...
            froms.forEach((from, index) => {
                if (typeof from === 'object' && !Array.isArray(from) && typeof from !== 'function') {
    ...
    createSub(callback) {
        ...
        if (typeof callback === 'function') {
            ...
            throw new Error('Invalid subquery type. Expected function or Builder instance.');
        }
    ...
    where(column, operator = null, value = null, andOr = 'and') {
        ...
        if (this.isRaw(value)) {
            ...
            return this.whereRaw(`${this.contactBacktick(column)} ${operatorUpper} ${value.value}`, [], andOrUpper);
        }
    ...
    whereSub(column, operator, callback = null, andOr = 'and') {
        ...
        [callback, operator] = this.prepareValueAndOperator(
            callback, operator, arguments.length === 2
        );
        ...
        return this.whereRaw(`${this.contactBacktick(column)} ${operatorUpper} (${sql})`, bindings, andOrUpper);
    }
    ...
    whereNull(column, operator = 'is', andOr = 'and', not = false) {
        ...
        return this.addQuery('wheres', `${prefix}${this.contactBacktick(column)} ${nullOperator}`);
    }
    ...
    whereIn(column, values = null, operator = 'in') {
        if (values instanceof Builder || typeof values === 'function') {
            ...
            return this.whereRaw(`${this.contactBacktick(column)} ${operator.toUpperCase()} (${sql})`, bindings);
        }
    ...
    orWhereIn(column, values = null, operator = 'in') {
        if (values instanceof Builder || typeof values === 'function') {
            const [sql, bindings] = this.createSub(values);
            return this.whereRaw(`${this.contactBacktick(column)} ${operator.toUpperCase()} (${sql})`, bindings, 'or');
        }
    ...
    whereBetween(column, values, not = false) {
        if (!Array.isArray(values) || values.length !== 2) {
            throw new Error('Between values must be an array with exactly 2 elements');
        }
    ...
    whereNested(callback, andOr = 'and') {
        ...
        if (nestedWheres.length > 0) {
            ...
            const existingWheres = this.getQuery('wheres.queries', []);
            const prefix = existingWheres.length === 0 ? '' : ` ${andOr.toUpperCase()} `;
     ...
     *   .get();
     */
    join(table, first, operator = null, second = null, type = 'inner', isWhere = false) {
     ...
     *   .get();
     */
    joinSub(table, as, first, operator = null, second = null, type = 'inner', isWhere = false) {
     ...
     * @returns {Builder} This builder instance
     */
    joinWhereSub(table, as, first, operator = null, second = null, type = 'inner') {
    ...
    joinWhen(condition, joinCallback, elseCallback = null) {
        const shouldJoin = typeof condition === 'function' ? condition() : condition;
    ...
    having(column, operator = null, value = null, andOr = 'and') {
        ...
        if (this.isRaw(value)) {
            const andOrUpper = andOr.toUpperCase();
            const operatorUpper = operator.toUpperCase();
            return this.havingRaw(`${this.contactBacktick(column)} ${operatorUpper} ${value.value}`, [], andOrUpper);
        }
    ...
    havingNested(callback, andOr = 'and') {
        ...
        if (nestedHavings.length > 0) {
            ...
            const existingHavings = this.getQuery('havings.queries', []);
            const prefix = existingHavings.length === 0 ? '' : ` ${andOr.toUpperCase()} `;

            return this.addQuery('havings', `${prefix}(${nestedSql})`, nestedBindings);
        }
    ...
    groupByWhen(condition, groupCallback, elseCallback = null) {
        const shouldGroup = typeof condition === 'function' ? condition() : condition;
    ...
    havingWhen(condition, havingCallback, elseCallback = null) {
        const shouldHave = typeof condition === 'function' ? condition() : condition;
    ...
    orderByWhen(condition, orderCallback, elseCallback = null) {
        const shouldOrder = typeof condition === 'function' ? condition() : condition;
    ...
    limitWhen(condition, limitValue, elseLimitValue = null) {
        const shouldLimit = typeof condition === 'function' ? condition() : condition;
            ...
            for (const operation of operations) {
                ...
                switch (type.toLowerCase()) {
                    case 'insert':
                        ...
                        break;
                    case 'update':
                        result = await this.clone().where(where || {}).update(data);
                        break;
                    case 'delete':
                        result = await this.clone().where(where || {}).delete();
                        break;
                    default:
    ...
    async createIndex(indexName, columns, options = {}) {
        ...
        const columnsStr = columns.map(col => this.contactBacktick(col)).join(', ');

        const sql = `CREATE ${uniqueClause} INDEX ${this.contactBacktick(indexName)} ON ${this.contactBacktick(tableName)} (${columnsStr}) ${typeClause}`.trim();
    ...
    getStatistics() {
        ...
        return {
            ...
            totalDuration,
            averageDuration,
            slowestQuery: logs.reduce((max, log) => log.duration > max.duration ? log : max),
            fastestQuery: logs.reduce((min, log) => log.duration < min.duration ? log : min)
        };
    }
     ...
     */
    async get(columns = ['*']) {
        if (!this.getQuery('columns.queries') || !this.arraysEqual(columns, ['*'])) {
            this.select(columns);
        }
    ...
    async update(data) {
        ...
        for (const [column, value] of Object.entries(data)) {
            if (typeof value === 'function' || value instanceof Builder) {
                ...
                // Handle raw expressions
                if (isNaN(column)) {
                    columns.push(this.raw(`${this.contactBacktick(column)} = (${value.value})`));
                } else {
                    columns.push(value);
                }
            } else {
    ...
    async insertSub(columns, query) {
        ...
        const compiledSql = this.grammar.compileInsert(this, this.arrayFlip(columns), sql);
        const allBindings = this.getBindings();
    ...
    async upsert(data, where = {}) {
        ...
        if (existing) {
            ...
            return { action: 'updated', affectedRows, record: { ...existing, ...data } };
        } else {
            // Insert new record
            const affectedRows = await this.clone().insert({ ...data, ...where });
            return { action: 'inserted', affectedRows, record: { ...data, ...where } };
        }
    }
    ...
    async increment(column, amount = 1) {
        const data = { [column]: this.raw(`${this.contactBacktick(column)} + ${amount}`) };
    ...
    async truncate() {
        if (!this.grammar.compileTruncate) {
            throw new Error('Truncate operation not supported by the current database grammar');
        }
     ...
     */
    clone() {
        const cloned = new this.constructor(this.connection, this.grammar, this.processor);
     ...
     */
        ...
        this.conditions.forEach(condition => {
            if (condition.type === 'callback') {
                ...
                const wheresSql = this.builder.grammar.compileWheres(
                    this.builder,
                    condition.condition
                ).replace(/^WHERE\s+/, '');
...
class JoinBuilder {
    ...
    on(first, operator = '=', second = null) {
        ...
        if (arguments.length === 2 && typeof operator === 'string' && second === null) {
            second = operator;
            operator = '=';
        }
    ...
    finalize() {
        ...
        if (this.table && typeof this.table === 'object' && this.table.raw === true) {
        ...
        if (this.isSubquery) {
            finalTable = table;
        } else if (this.table && typeof this.table === 'object' && this.table.raw === true) {

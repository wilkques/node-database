...
export default class Grammar implements GrammarInterface {
    ...
    compileSelect(query: any): string {
        ...
        if (query.components && (!query.queries || Object.values(query.queries).every((q: any) => !q.queries || q.queries.length === 0))) {
            actualQuery = {
                queries: {
                    columns: {
    ...
    protected compileJoins(query: any): string {
        ...
        return query.queries.joins.queries.map((join: any) => {
    ...
    protected compileGroups(query: any): string {
        ...
        const groups = query.queries.groups.queries.map((group: any) => this.wrap(group));
    ...
    protected compileHavings(query: any): string {
        ...
        const havings = query.queries.havings.queries.map((having: any, index: number) => {
        ...
        if (args.length === 1) {
            ...
            if (Array.isArray(arg)) {
                const parts = arg.map(item =>
                    typeof item === 'string' ? this.wrapSingleIdentifier(item) : String(item)
                );
                return parts.join('.');
            }

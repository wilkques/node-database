...
export default class Grammar {
    ...
    compileWheres(query, wheres) {
        ...
        const compiledWheres = wheres.map((where, index) => {
            ...
            switch (where.type) {
                ...
                case 'nested':
                    const nestedWheres = this.compileWheres(where.query, where.query.components.wheres);
    ...
    compileHavings(query, havings) {
        ...
        const compiledHavings = havings.map((having, index) => {
    ...
    compileInsert(query, data) {
        ...
        if (Array.isArray(data)) {
            ...
            const valuesList = data.map(() => `(${columns.map(() => '?').join(', ')})`).join(', ');
    ...
    compileUpdate(query, data) {
        ...
        const assignments = Object.keys(data).map(column => {
    ...
    wrapColumn(column) {
        ...
        if (column.includes('.')) {
            ...
            return parts.map(part => part === '*' ? part : this.wrapValue(part)).join('.');
        }

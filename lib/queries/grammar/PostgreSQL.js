...
export default class PostgreSQLGrammar extends Grammar {
    ...
    compileWheres(query, wheres) {
        ...
        const compiled = wheres.map((where, index) => {
            const boolean = index === 0 ? '' : `${(where.boolean || 'AND').toUpperCase()} `;
            ...
            switch (where.type) {
                ...
                case 'in':
                    const placeholders = where.values.map(() => this.getParameterPlaceholder()).join(', ');
                    return `${boolean}${this.wrapColumn(where.column)} IN (${placeholders})`;

                case 'not_in':
                    const notInPlaceholders = where.values.map(() => this.getParameterPlaceholder()).join(', ');
                ...
                case 'nested':
                    ...
                    return nestedSql ? `${boolean}(${nestedSql.replace(/^WHERE\s+/i, '')})` : '';
    ...
    compileUpsert(query, data, conflicts = [], updateData = {}) {
        ...
        if (conflicts.length > 0) {
            const conflictColumns = conflicts.map(col => this.wrapColumn(col)).join(', ');
            sql += ` ON CONFLICT (${conflictColumns})`;

            if (Object.keys(updateData).length > 0) {
                const updates = Object.keys(updateData).map(column => {
     ...
     */
    compileLockForUpdate(query, lockMode = 'UPDATE', tables = [], noWait = false) {

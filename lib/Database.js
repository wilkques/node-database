...
class Database {
     ...
     * @since 1.0.0
     */
    static async connect(driver, host = 'localhost', username = '', password = '', database = '', port = null, charset = 'utf8mb4') {
        let config;
    ...
    static async _createConnection(config) {
        ...
        try {
            switch (config.driver) {
                case 'mysql':
                    const { default: MySqlDriver } = await import('./connections/drivers/mysql.js');
                    Driver = MySqlDriver;
                    break;
                case 'postgres':
                case 'postgresql':
                    const { default: PostgreSQLDriver } = await import('./connections/drivers/postgres.js');
                    Driver = PostgreSQLDriver;
                    break;
                case 'sqlite':
                    const { default: SQLiteDriver } = await import('./connections/drivers/sqlite.js');
                    Driver = SQLiteDriver;
                    break;
                default:
                    throw new Error(`Unsupported driver [${config.driver}]`);
    ...
    static async _createGrammar(driver) {
        try {
            switch (driver) {
                case 'mysql':
                    const { default: MySqlGrammar } = await import('./queries/grammar/MySQL.js');
                    return new MySqlGrammar();
                case 'postgres':
                case 'postgresql':
                    const { default: PostgreSQLGrammar } = await import('./queries/grammar/PostgreSQL.js');
                    return new PostgreSQLGrammar();
                case 'sqlite':
                    const { default: SQLiteGrammar } = await import('./queries/grammar/SQLite.js');

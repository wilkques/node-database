...
async function basicExample() {
    ...
    try {
        ...
        const userProfiles = await db.table('users as u')
        ...
        const activeUserIds = await db.table('users')
            .select('id')
            .whereIn('id', (query) => {
                query.table('user_sessions')
                    .select('user_id')
                    .where('last_activity', '>', new Date(Date.now() - 24 * 60 * 60 * 1000));
            })
            .get();
        ...
        console.log('SQL for previous SELECT:', db.table('users').select('*').toSql());
        console.log();
...
async function alternativeConnections() {
    ...
    try {
        ...
        const stringDb = await Database.connect('mysql', 'localhost', 'root', '', 'test');

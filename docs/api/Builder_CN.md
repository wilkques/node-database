...
db.table("orders")
  .where("paid", true)
  .where((q) => {
    q.where("shipped", true).orWhere("processing", true);
  })
  .get();
...
```javascript
db.table("users").where("id", "=", db.table("admins").select("user_id")).get();
```

**OR条件：**

```javascript
db.table("users").where("status", "active").orWhere("premium", true).get();
...
```javascript
db.table("users").where("status", "active").orderBy("created_at").get();
```

**单列降序：**

```javascript
db.table("posts").orderBy("created_at", "desc").limit(10).get();
```
...
```javascript
db.table("products").offset(20).limit(10).orderBy("id").get();
```

**单条记录（limit 1）：**

```javascript
db.table("users").where("email", "user@example.com").limit(1).first(); // 或使用 first() 替代
```
...
```javascript
const active = await db.table("users").where("status", "active").get();
```
...
```javascript
const exists =
  (await db.table("users").where("email", "test@example.com").first()) !== null;
...
```javascript
const exists =
  (await db.table("users").where("email", "test@example.com").count()) > 0;
...
```javascript
const userCount = await db.table("users").where("role", "admin").count();
```
...
// 使用子查询JOIN
const recentUserOrders = await db
  .table("users")
...
const allUserStats = await db
  ...
  .select(["users.name", db.raw("COALESCE(stats.count, 0) as order_count")])
  .orderBy("order_count", "desc")
  .get();
```
...
```javascript
// 按地区和产品分组统计销售额
const salesByRegionProduct = await db
  .table("sales")
  .select([
    "region",
    "product",
...
const allSales = await db
  ...
  .unionAll((q) =>
    q.table("sales_region2").select(["date", "amount", "product"]),
  )
  .unionAll((q) =>
    q.table("sales_region3").select(["date", "amount", "product"]),
  )
  .orderBy("date", "desc")
  .get();
```
...
db.table("sales")
  .select(
    ...
    db
      .case()
      .when(
        (q) => q.where("amount", ">", 10000).where("region", "north"),
        "Premium-North",
      )
      .when(
        (q) => q.where("amount", ">", 10000).where("region", "south"),
        "Premium-South",
      )
...
// 或省略 else()
db.case("type").when("A", "Type A").when("B", "Type B").end("type_name");
```
...
db.case()
  ...
  .when(
    (q) =>
      q.where("created_at", ">", db.raw("DATE_SUB(NOW(), INTERVAL 30 DAY)")),
    "New User",
  )
  .else("Inactive")
  .end("user_status");
...
```javascript
// ✅ 好：简单CASE - 当评估单列时使用
db.case("status").when("active", 1).when("inactive", 0).end("status_code");
...
const bad = await db
  .table("orders")
  .where(db.case("status").when("completed", 1).else(0).end())
  .get();
...
const good = await db
  ...
  .select(
    "*",
    db.case("status").when("completed", "Done").else("Pending").end("display"),
  )
  .where("status", "completed")
  .get();
```
...
const orders = await db
  ...
  .select(
    ...
    db
      .case()
      .when(
        (q) => q.where("total", ">", 1000).where("status", "completed"),
        "Premium Completed",
      )
      .when(
        (q) => q.where("total", ">", 1000).where("status", "pending"),
        "Premium Pending",
      )
      .when(
        (q) => q.where("total", "<=", 1000).where("status", "completed"),
...
const users = await db
  ...
  .select(
    ...
    db
      ...
      .when(
        (q) => q.where("premium", true).where("verified", true),
        "Premium Member",
      )
...
const inserted = await db
  .table("users_archive")
  .insertSub(["id", "name", "email", "created_at"], (q) =>
    q
      .table("users")
...
const inserted = await db
  .table("user_reports")
  .insertSub(["user_id", "total_orders", "total_spent", "report_date"], (q) =>
    q
      .table("users")
      .join("orders", "users.id", "orders.user_id")
      .select([
...
```javascript
const inserted = await db
  .table("products_normalized")
  .insertSub(["product_id", "name", "price_usd"], (q) =>
    q
      .table("products")
      .select(["id", "name", db.raw("ROUND(price * 1.1, 2) as price_usd")])
      .where("published", true),
  );
```
...
```javascript
const deleted = await db.table("users").where("id", 123).delete();
...
```javascript
const affected = await db.table("pages").where("id", 42).increment("views");
...
const result = await db
  .table("user_settings")
  .upsert({ theme: "dark", notifications: true }, { user_id: 42 });
...
const result = await db
  .table("user_settings")
  .insertOrUpdate({ theme: "dark", notifications: true }, { user_id: 42 });
...
const orders = await db
  ...
  .where("total", ">", 500)
  .where((q) => {
    q.where("payment_method", "credit_card").orWhere(
      "payment_method",
      "paypal",
    );
  })

...
// 大于等于
const enoughStock = await db.table("products").where("stock", ">=", 10).get();
...
// 小于等于
const lowScores = await db.table("scores").where("value", "<=", 50).get();
...
const query = await db
  ...
  .where("status", "active")
  .where((q) => {
    q.where("role", "admin").orWhere("premium", true);
  })
  .get();
...
const results = await db
  ...
  .where((q) => {
    q.where("total", ">", 100).orWhere((qq) => {
      qq.where("discount", ">", 20).where("user_premium", true);
    });
  })
  .get();
...
const users = await db.table("users").whereIn("id", userIds).get();

// 查询role在多个值中的用户
const roles = ["admin", "moderator", "user"];
const staff = await db.table("users").whereIn("role", roles).get();
```
...
// 查询id不在列表中的用户
const userIds = [1, 2, 3];
const otherUsers = await db.table("users").whereNotIn("id", userIds).get();
```
...
```javascript
// 查询email为NULL的用户
const noEmail = await db.table("users").where("email", null).get();
// 或者
const noEmailAlt = await db.table("users").whereNull("email").get();

// 查询email不为NULL的用户
const hasEmail = await db.table("users").whereNotNull("email").get();
```
...
// 条件求和
const userTotal = await db.table("orders").where("user_id", 1).sum("amount");
...
```javascript
// 获取所有不同的城市
const cities = await db.table("users").distinct("city").get();

// 等同于SQL: SELECT DISTINCT city FROM users

// 获取多列的不同值组合
const distinctCombos = await db
  .table("orders")
...
const usersWithOrders = await db
  ...
  .select("id", "name", "email")
  .whereIn("id", (query) => {
    query.table("orders").select("user_id").distinct("user_id");
  })
  .get();
...
```javascript
// 查询有订单的用户（使用EXISTS）
const usersWithOrdersExists = await db
  .table("users")
  .select("id", "name")
  .where((query) => {
    query
...
```javascript
// 获取所有用户的ID列表
const userIds = await db.table("users").where("status", "active").pluck("id");

console.log(userIds);
// 输出: [1, 2, 3, 4, 5, ...]

// 获取用户名列表
const names = await db.table("users").orderBy("name").pluck("name");
```
...
```javascript
// 获取ID到名字的映射
const userMap = await db.table("users").lists("id", "name");

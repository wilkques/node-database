...
// Using OR conditions
const premiumOrAdmin = await db.table('users')
    .where('role', 'admin')
...
const fullData = await db.table('users as u')
...
// Conditional update
const statusUpdateResult = await db.table('users')
    .where('age', '<', 18)
    .update({ status: 'minor' });

// Increment field value
const incrementResult = await db.table('users')
    .where('id', 1)
...
// Conditional delete
const expiredDeleteResult = await db.table('sessions')
...
// Using functions
const usersWithAge = await db.table('users')
...
```javascript
// IN subquery
const activeUserOrders = await db.table('orders')
...
const deptStats = await db.table('employees')
    .select('department')

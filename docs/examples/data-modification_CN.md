...
async function bulkInsertUsers(allUsers, batchSize = 1000) {
    for (let i = 0; i < allUsers.length; i += batchSize) {
        ...
        console.log(`已插入 ${Math.min(i + batchSize, allUsers.length)} / ${allUsers.length}`);
    }
}
...
const result = await db.table('products')
    .where('id', 5)
    .update({
        avg_rating: db.raw(`(SELECT AVG(rating) FROM reviews WHERE product_id = 5)`),
        updated_at: new Date()
    });
```
...
const result = await db.table('logs')
...
```javascript
// ✅ 推荐：软删除（标记为已删除，不实际删除）
const softDelete = await db.table('users')
    .where('id', 1)
    .update({
        deleted_at: new Date()
    });
...
// 增加指定数量
const result2 = await db.table('users')
    .where('id', 1)
    .increment('login_count', 5);

// 增加多个字段
const result3 = await db.table('users')
    .where('id', 1)
    .increment({
        login_count: 1,
        points: 10
...
// 减少指定数量
const result2 = await db.table('products')
    .where('id', 5)
    .decrement('stock', 2);

// 减少多个字段
const result3 = await db.table('orders')
    .where('id', 1)
    .decrement({
        quantity: 1,
        total_amount: 9.99
...
async function bulkInsertAndGetIds(records) {
    ...
    // 获取插入的所有记录
    const inserted = await db.table('products')
...
async function cleanupOldData() {
    ...
    try {
        ...
        // 1. 清理过期会话
        const sessionCount = await transaction.table('sessions')
        ...
        // 2. 归档旧日志
        const logCount = await transaction.table('logs')
        ...
        // 3. 删除未完成的临时订单
        const tempOrderCount = await transaction.table('orders')
            .where('status', 'pending')
            .where('created_at', '<', ninetyDaysAgo)
            .delete();

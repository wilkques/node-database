...
const statusUpdate = await db.table('users')
...
const specificUpdate = await db.table('users')
    .whereIn('id', [1, 3, 5, 7])
    .update({
        category: 'premium',
        updated_at: new Date()
...
const joinUpdate = await db.table('users')
...
const calculatedUpdate = await db.table('products')
...
const conditionalUpdate = await db.table('users')
    .update({
        membership_level: db.case()
...
const cleanupResult = await db.table('users')
...
const specificDelete = await db.table('temporary_logs')
...
const relatedDelete = await db.table('user_sessions')
...
const incrementResult = await db.table('posts')
    .where('id', 1)
    .increment('view_count');

// Increment by specific amount
const customIncrement = await db.table('users')
    .where('id', 1)
    .increment('points', 50);

// Increment multiple fields
const multiIncrement = await db.table('statistics')
    .where('id', 1)
    .increment({
        page_views: 1,
        unique_visitors: 1,
...
```javascript
// Decrement inventory
const decrementResult = await db.table('products')
    .where('id', 10)
    .decrement('stock', 2);

// Decrement with minimum check
const safeDecrement = await db.table('products')
    .where('id', 10)
    .where('stock', '>=', 2)
    .decrement('stock', 2);
```
...
const combinedUpdate = await db.table('users')
    .where('id', 1)
...
// Usage
const result = await customUpsert('users', {
...
async function safeUpdateUser(userId, updateData, expectedVersion) {
    ...
    try {
        ...
        const currentUser = await transaction.table('users')
            .where('id', userId)
            .first();
        ...
        const result = await transaction.table('users')
            .where('id', userId)

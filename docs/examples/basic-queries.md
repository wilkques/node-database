...
// Using array syntax
const usersArray = await db.table('users')
    .select(['id', 'name', 'email'])
    .get();

// With table alias
const usersAlias = await db.table('users as u')
...
// Not equal
const nonActiveUsers = await db.table('users')
    .where('status', '!=', 'active')
    .get();
```
...
```javascript
// Simple OR
const adminOrModerator = await db.table('users')
    .where('role', 'admin')
    .orWhere('role', 'moderator')
    .get();

// Complex OR with grouping
const eligibleUsers = await db.table('users')
    .where('status', 'active')
...
const emailSearch = await db.table('users')
...
// Descending
const usersDesc = await db.table('users')
...
const priceStats = await db.table('products')
    .select(
...
const usersByStatus = await db.table('users')
    .select('status')
    .count('* as user_count')
    .groupBy('status')
    .get();
...
const salesByRegionAndMonth = await db.table('orders')
    .select('region', 'month')
...
const significantSales = await db.table('orders')
    .select('customer_id')
...
```javascript
// Unique statuses
const uniqueStatuses = await db.table('users')
    .distinct('status')
    .get();

// Multiple distinct columns
const uniqueLocations = await db.table('users')
...
const uniqueCustomers = await db.table('orders')
...
const activeCustomers = await db.table('users')

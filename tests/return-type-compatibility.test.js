...
describe('Return Type Compatibility', () => {
    ...
    test('All JOIN method variants return JoinBuilder', () => {
        ...
        const innerJoin = builder.table('posts').join('users', 'posts.user_id', '=', 'users.id');
        const leftJoin = builder.table('posts').leftJoin('users', 'posts.user_id', '=', 'users.id');
        const rightJoin = builder.table('posts').rightJoin('users', 'posts.user_id', '=', 'users.id');
    ...
    test('Subquery JOIN methods return JoinBuilder', () => {
        ...
        const joinSub = builder.table('posts').joinSub(
        ...
        const leftJoinSub = builder.table('posts').leftJoinSub(
            (q) => q.table('user_stats').select('user_id', 'total_orders'),
            'stats',
            'posts.user_id',
            '=',

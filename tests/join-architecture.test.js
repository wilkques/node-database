...
describe('JoinBuilder Architecture', () => {
    ...
    test('JoinBuilder should process standard JOIN', () => {
        ...
        const joinBuilder = new JoinBuilder(mockConnection);
        const result = joinBuilder.processJoin('users', 'posts.user_id', '=', 'users.id', 'inner');
    ...
    test('JoinBuilder should handle subquery joins', () => {
        const joinBuilder = new JoinBuilder(mockConnection, 'subquery_alias', 'left');
...
describe('JOIN Architecture Integration Tests', () => {
    ...
    test('Complex multi-JOIN query with all JOIN types', () => {
        ...
        const result = builder.table('posts')
            ...
            .crossJoin('settings')
            .where('posts.status', 'published')
            .select(['posts.title', 'users.name', 'profiles.bio', 'categories.name as category'])
    ...
    test('Complex subquery JOIN like user example', () => {
        ...
        const result = builder.table('GameMaster gm')
            .leftJoin('GameSpec gs', 'gm.id', '=', 'gs.id_GameMaster')
            .leftJoinSub(
                (query) => {
                    query.table('PlatformProduct').select('id', 'id_ProductMaster', 'id_PlatformMaster');
                },
                'pp',
                'pp.id',
                '=',
            ...
            .leftJoinSub(
                (query) => {
                    query.table('ProductMaster').select('id', 'code', 'name').where('companyindex', 1);
                },
                'pm',
                'pm.id',
                '=',
    ...
    test('Performance test: Complex JOIN chain with many tables', () => {
        ...
        const result = builder.table('posts')
            ...
            }, 'stats', 'posts.id', '=', 'stats.post_id')
            .leftJoinSub((q) => {
                q.table('user_badges').select('user_id', 'badge_name').where('active', true);
            }, 'badges', 'users.id', '=', 'badges.user_id')
    ...
    test('Three-layer architecture validation', () => {
        ...
        expect(sql).toMatch(/SELECT \* FROM `posts` INNER JOIN `users` ON `posts`\.`user_id` = `users`\.`id`/);

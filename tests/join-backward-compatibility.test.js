...
describe('JOIN Backward Compatibility', () => {
    ...
    const testCases = [
        {
            ...
            code: (builder) => builder.join('users', 'posts.user_id', '=', 'users.id'),
            expectedJoinType: 'inner'
        },
        {
            name: 'Left join with default operator',
            code: (builder) => builder.leftJoin('profiles', 'users.id', 'profiles.user_id'),
            expectedJoinType: 'left'
        },
        {
            name: 'Right join explicit operator',
            code: (builder) => builder.rightJoin('categories', 'posts.category_id', '=', 'categories.id'),
        ...
        {
            name: 'Full join',
            code: (builder) => builder.fullJoin('audit_log', 'posts.id', '=', 'audit_log.post_id'),
    ...
    test('Subquery JOINs should maintain compatibility', () => {
        ...
        const result = builder.table('posts')
            .joinSub(
    ...
    test('Subquery JOIN with Builder instance should work', () => {
        ...
        subQuery.table('user_stats').select('user_id', 'total_posts').where('active', true);
    ...
    test('Complex chaining should work exactly as before', () => {
        ...
        const result = builder.table('posts')
            .select(['posts.id', 'posts.title', 'users.name', 'categories.name as category'])
            .join('users', 'posts.user_id', '=', 'users.id')
    ...
    test('All Builder methods should be available on JoinBuilder result', () => {
        ...
        const joinResult = builder.table('posts').join('users', 'posts.user_id', '=', 'users.id');
    ...
    test('JOIN method signatures match original implementation', () => {
        ...
        tests.forEach((testFn, index) => {
            try {
                ...
                throw new Error(`Join signature test ${index + 1} failed: ${error.message}`);
            }
        });
    });
    ...
    test('SubJoin method signatures match original implementation', () => {
        ...
        tests.forEach((testFn, index) => {
            try {
                const result = testFn();
                expect(result).toBeInstanceOf(JoinBuilder);
                expect(result.components.joins[result.components.joins.length - 1].isSubquery).toBe(true);
            } catch (error) {
                throw new Error(`SubJoin signature test ${index + 1} failed: ${error.message}`);
            }
        });
    });
});
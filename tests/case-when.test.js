...
describe("CASE WHEN Expressions", () => {
  ...
  describe("Simple CASE (column-based)", () => {
    test("should create simple CASE with column and values", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE status WHEN ? THEN ? WHEN ? THEN ? ELSE ? END",
      );
    ...
    test("should create simple CASE with alias", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE priority WHEN ? THEN ? WHEN ? THEN ? END as priority_text",
      );
      expect(caseExpr.bindings).toEqual([
        "high",
        "High Priority",
    ...
    test("should handle case without ELSE clause", () => {
      const caseExpr = builder.case("status").when("active", "Active").end();
  ...
  describe("Searched CASE (condition-based)", () => {
    test("should create searched CASE without column", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN total > 1000 THEN ? WHEN total > 500 THEN ? ELSE ? END",
      );
      expect(caseExpr.bindings).toEqual([
        "Large Order",
        "Medium Order",
    ...
    test("should handle complex conditions", () => {
      ...
      expect(caseExpr.value).toBe(
        'CASE WHEN status = "active" AND created_at > "2024-01-01" THEN ? WHEN status = "inactive" THEN ? ELSE ? END',
      );
  ...
  describe("Function-based CASE (parameter binding)", () => {
    ...
    test("should handle multiple function-based conditions", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN `id` IN (?, ?, ?) THEN ? WHEN `level` > ? THEN ? ELSE ? END",
      );
    ...
    test("should handle complex WHERE conditions in functions", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN `active` = ? AND `created_at` > ? THEN ? ELSE ? END",
      );
      expect(caseExpr.bindings).toEqual([
        true,
        "2024-01-01",
  ...
  describe("Function-based CASE (subquery)", () => {
    test("should create CASE with subquery conditions", () => {
      ...
      expect(caseExpr.raw).toBe(true);
      expect(caseExpr.value).toBe(
        "CASE WHEN id IN (SELECT `id` FROM `ActiveUsers` WHERE `status` = ?) THEN ? ELSE ? END",
      );
    ...
    test("should handle complex subqueries", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN user_id IN (SELECT `user_id` FROM `VipUsers` WHERE `level` > ? AND `active` = ?) THEN ? ELSE ? END",
      );
  ...
  describe("Mixed CASE expressions", () => {
    test("should handle mixed parameter binding and subquery forms", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN `id` IN (?, ?, ?) THEN ? WHEN priority IN (SELECT `user_id` FROM `VipUsers` WHERE `level` > ?) THEN ? ELSE ? END",
      );
      expect(caseExpr.bindings).toEqual([1, 2, 3, "High", 5, "VIP", "Normal"]);
    });
    ...
    test("should handle multiple subquery conditions", () => {
      ...
      expect(caseExpr.value).toBe(
        "CASE WHEN user_id IN (SELECT `id` FROM `AdminUsers`) THEN ? WHEN user_id IN (SELECT `user_id` FROM `VipUsers` WHERE `active` = ?) THEN ? ELSE ? END",
      );
  ...
  describe("CASE in UPDATE statements", () => {
    test("should work in UPDATE with parameter binding", () => {
      ...
      const sql = updateQuery.grammar.compileUpdate(updateQuery, {
        status: caseExpr,
      });
      const bindings = updateQuery.getUpdateBindings({ status: caseExpr });

      expect(sql).toBe(
        "UPDATE `users` SET `status` = CASE WHEN `id` IN (?, ?) THEN ? ELSE ? END WHERE `id` IN (?, ?, ?)",
      );
    ...
    test("should work in UPDATE with subquery", () => {
      ...
      expect(sql).toContain(
        "UPDATE `users` SET `status` = CASE WHEN id IN (SELECT `user_id` FROM `VipUsers` WHERE `level` > ?) THEN ? ELSE ? END",
      );
  ...
  describe("SQL binding parameter validation", () => {
    ...
    test("should handle complex mixed conditions correctly", () => {
      const caseExpr = builder
        .case()
        .when(
          (query) => query.where("total", ">", 1000).where("status", "active"),
          "Premium",
        )

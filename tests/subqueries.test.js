...
describe("Subquery Functionality", () => {
    ...
    describe("WHERE Subqueries", () => {
        test("should create WHERE IN subquery", () => {
            const query = builder.table('users')
                .where('id', 'IN', query => {
            ...
            expect(sql).toBe('SELECT * FROM `users` WHERE `id` IN (SELECT `user_id` FROM `orders` WHERE `status` = ?)');
        ...
        test("should create WHERE NOT EXISTS subquery", () => {
            ...
            expect(sql).toBe('SELECT * FROM `users` WHERE NOT EXISTS (SELECT * FROM `orders` WHERE `orders`.`user_id` = `users`.`id`)');
        });
        ...
        test("should handle OR WHERE IN subquery", () => {
            ...
            expect(sql).toBe('SELECT * FROM `users` WHERE `status` = ? OR `id` IN (SELECT `user_id` FROM `orders` WHERE `total` > ?)');
        ...
        test("should handle complex nested subqueries", () => {
            ...
            expect(sql).toContain('WHERE `product_id` IN (SELECT `id` FROM `products`');
    ...
    describe("SELECT Subqueries", () => {
        test("should create SELECT with subquery column", () => {
            const query = builder.table('users')
                ...
                .selectSub(
                    subQuery => {
                        subQuery.table('orders').select('COUNT(*)').where('orders.user_id', '=', 'users.id');
                    },
                    'order_count'
                );
            ...
            expect(sql).toBe('SELECT `id`, `name`, (SELECT COUNT(*) FROM `orders` WHERE `orders`.`user_id` = `users`.`id`) AS `order_count` FROM `users`');
        });
        ...
        test("should handle multiple SELECT subqueries", () => {
            const query = builder.table('users')
                .select('id', 'name')
                .selectSub(
                    subQuery => subQuery.table('orders').select('COUNT(*)').where('orders.user_id', '=', 'users.id'),
                    'order_count'
                )
                .selectSub(
                    subQuery => subQuery.table('reviews').select('AVG(`rating`)').where('reviews.user_id', '=', 'users.id'),
                    'avg_rating'
                );
        ...
        test("should handle SELECT subquery with conditions", () => {
            const query = builder.table('categories')
                .select('name')
                .selectSub(
                    subQuery => {
                        subQuery.table('products')
                            .select('COUNT(*)')
            ...
            expect(sql).toContain('WHERE `products`.`category_id` = `categories`.`id`');
    ...
    describe("JOIN Subqueries", () => {
        test("should create JOIN with subquery", () => {
            ...
            expect(sql).toContain('INNER JOIN (SELECT `user_id`, COUNT(*) as order_count FROM `orders` GROUP BY `user_id`) AS `order_stats`');
        ...
        test("should handle complex JOIN subquery with conditions", () => {
            const query = builder.table('products')
                .leftJoinSub(
                    subQuery => {
            ...
            expect(sql).toContain('LEFT JOIN (SELECT `product_id`, AVG(rating) as avg_rating, COUNT(*) as review_count');
    ...
    describe("FROM Subqueries", () => {
        test("should create FROM subquery", () => {
            ...
            expect(sql).toBe('SELECT * FROM (SELECT `user_id`, SUM(total) as total_spent FROM `orders` GROUP BY `user_id`) AS `user_totals` WHERE `total_spent` > ?');
        ...
        test("should handle FROM subquery with JOIN", () => {
            ...
            expect(sql).toContain('FROM (SELECT `product_id`, SUM(quantity) as total_sold FROM `sales`');
    ...
    describe("INSERT with Subqueries", () => {
        test("should create INSERT with SELECT subquery", () => {
            ...
            expect(result.sql).toContain('(`user_id`, `total_orders`, `total_spent`)');
            expect(result.sql).toContain('SELECT `user_id`, COUNT(*) as total_orders, SUM(total) as total_spent');
    ...
    describe("ORDER BY and GROUP BY with Subqueries", () => {
        test("should handle ORDER BY with subquery", () => {
            ...
            expect(sql).toBe('SELECT * FROM `users` ORDER BY (SELECT COUNT(*) FROM orders WHERE orders.user_id = users.id) DESC');
        });
        ...
        test("should handle complex ordering with multiple subqueries", () => {
            const query = builder.table('products')
                ...
                .orderByRaw('(SELECT COUNT(*) FROM orders_items WHERE orders_items.product_id = products.id) DESC');
    ...
    describe("Complex Combinations", () => {
        test("should handle multiple subquery types in one query", () => {
            const query = builder.table('users')
                ...
                .orderByRaw('(SELECT MAX(created_at) FROM orders WHERE orders.user_id = users.id) DESC');
        ...
        test("should handle deeply nested subqueries", () => {
            const query = builder.table('categories')
                .whereIn('id', categoryQuery => {
                    categoryQuery.table('products')
                        .select('category_id')
                        .whereIn('id', productQuery => {
                            productQuery.table('order_items')
                                .select('product_id')
                                .whereIn('order_id', orderQuery => {
                                    orderQuery.table('orders')
                                        .select('id')
                                        .where('status', 'completed')
    ...
    describe("Subquery SQL Generation Validation", () => {
        test("should have correct placeholder and binding counts", () => {
            const query = builder.table('users')
                .selectSub(
                    subQuery => subQuery.table('orders').select('COUNT(*)').where('user_id', '=', 'users.id').where('status', 'completed'),
                    'completed_orders'
                )
                .whereIn('id', subQuery => {
                    subQuery.table('purchases').select('user_id').where('amount', '>', 100);
                })
                .whereExists(subQuery => {
                    subQuery.table('reviews').where('user_id', '=', 'users.id').where('rating', '>', 4);
                });

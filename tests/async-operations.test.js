...
describe("Async Operations", () => {
  ...
  describe("SELECT Operations SQL Generation", () => {
    ...
    test("should generate correct EXISTS SQL", () => {
      ...
      const sql = subBuilder.from('users').where('email', 'john@example.com').toSql();
    ...
    test("should generate correct aggregate function SQL", () => {
      ...
      const maxSql = maxBuilder.from('products').toSql().replace('SELECT *', 'SELECT MAX(`price`) as `max`');
      const minSql = minBuilder.from('products').toSql().replace('SELECT *', 'SELECT MIN(`price`) as `min`');
      const sumSql = sumBuilder.from('orders').toSql().replace('SELECT *', 'SELECT SUM(`amount`) as `sum`');
      const avgSql = avgBuilder.from('products').toSql().replace('SELECT *', 'SELECT AVG(`rating`) as `avg`');
  ...
  describe("INSERT Operations SQL Generation", () => {
    ...
    test("should generate correct INSERT SQL for multiple records", () => {
      ...
      expect(sql).toBe('INSERT INTO `users` (`name`, `email`) VALUES (?, ?), (?, ?)');
      expect(bindings).toEqual(['John', 'john@example.com', 'Jane', 'jane@example.com']);
    });
  });

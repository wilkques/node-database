...
describe('PostgreSQLGrammar - Enhanced Features', () => {
    ...
    describe('wrapValue', () => {
        ...
        it('should escape double quotes in values', () => {
            expect(grammar.wrapValue('column"with"quotes')).toBe('"column""with""quotes"');
        });
    });
    ...
    describe('compileInsertReturning', () => {
        it('should compile INSERT with RETURNING', () => {
            ...
            grammar.compileInsert = jest.fn(() => 'INSERT INTO "users" ("name", "email") VALUES (?, ?)');
            grammar.wrapColumn = jest.fn((col) => `"${col}"`);

            const result = grammar.compileInsertReturning(mockQuery, {}, ['id']);
            expect(result).toBe('INSERT INTO "users" ("name", "email") VALUES (?, ?) RETURNING "id"');
        });
    });

    describe('jsonExtract', () => {
        it('should generate JSON extraction expressions', () => {
            expect(grammar.jsonExtract('data', 'user.name')).toBe('"data" -> \'user.name\'');
            expect(grammar.jsonExtract('config', 'settings', '->>')).toBe('"config" ->> \'settings\'');
        });
    });
});
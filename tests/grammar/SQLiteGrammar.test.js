...
describe("SQLiteGrammar - Enhanced Features", () => {
  ...
  describe("compileUpsert", () => {
    it("should use INSERT OR REPLACE for upsert", () => {
      const mockQuery = {};
      grammar.compileReplace = jest.fn(
  ...
  describe("dateFunction", () => {
    it("should compile date functions", () => {
      expect(grammar.dateFunction("date", "created_at")).toBe(
        "DATE([created_at])",
      );
      expect(grammar.dateFunction("time", "created_at")).toBe(
        "TIME([created_at])",
      );
      expect(grammar.dateFunction("datetime", "created_at")).toBe(
        "DATETIME([created_at])",
      );
    });
  ...
  describe("compilePragma", () => {
    ...
    it("should compile PRAGMA statement with value", () => {
      expect(grammar.compilePragma("journal_mode", "WAL")).toBe(

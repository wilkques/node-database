...
describe('Grammar Performance Tests', () => {
    ...
    test.each(grammars)('should compile simple queries efficiently - $name', ({ grammar }) => {
        ...
        console.log(`${grammar.constructor.name} - Average compilation time: ${avgTime.toFixed(3)}ms`);
        expect(avgTime).toBeLessThan(2); // Should be under 2ms per simple query
    });

    test.each(grammars)('should compile complex queries efficiently - $name', ({ grammar }) => {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            builder.select(['users.id', 'users.name', 'users.email', 'profiles.bio', 'profiles.avatar'])
                .from('users')
        ...
        console.log(`${grammar.constructor.name} - Complex query average time: ${avgTime.toFixed(3)}ms`);
        expect(avgTime).toBeLessThan(5); // Should be under 5ms per complex query
    });

    test.each(grammars)('should handle subquery compilation efficiently - $name', ({ grammar }) => {
        ...
        console.log(`${grammar.constructor.name} - Complex query 2 average time: ${avgTime.toFixed(3)}ms`);
    ...
    test('should handle concurrent query compilation', async () => {
        ...
        console.log(`Concurrent compilation - Average time per query: ${avgTimePerQuery.toFixed(3)}ms`);

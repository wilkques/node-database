...
describe("Transaction Handling", () => {
  ...
  describe("Builder Operations in Transaction Context", () => {
    ...
    test("should generate correct UPDATE SQL for transaction", () => {
      ...
      const sql = builder.grammar.compileUpdate(builder, { name: 'John Updated' });
  ...
  describe("Transaction Error Handling", () => {
    test("should handle query error simulation", async () => {
      ...
      await expect(mockConnection.query('SELECT * FROM users'))
    ...
    test("should handle connection lost simulation", async () => {
      ...
      await expect(mockConnection.commit())
    ...
    test("should handle rollback failure simulation", async () => {
      ...
      await expect(mockConnection.rollback())
  ...
  describe("Complex Operations in Transaction Context", () => {
    test("should generate SQL for multiple operations", () => {
      ...
      const insertSql = builder1.grammar.compileInsert(builder1, [{ name: 'John', email: 'john@test.com' }]);
      const insertSql2 = builder2.grammar.compileInsert(builder2, [{ user_id: 1, bio: 'Test bio' }]);
      const updateSql = builder3.grammar.compileUpdate(builder3, { verified: true });

      expect(insertSql).toBe('INSERT INTO `users` (`name`, `email`) VALUES (?, ?)');
      expect(insertSql2).toBe('INSERT INTO `users` (`user_id`, `bio`) VALUES (?, ?)');
      expect(updateSql).toBe('UPDATE `users` SET `verified` = ? WHERE `id` = ?');
    });
